...
func (m *MessagingClient) ConnectToBroker(connectionString string) {
	if connectionString == "" {
		logrus.Errorln("Cannot initialize connection to broker, connectionString not set. Have you initialized?")
	...
	if err != nil {
		logrus.Errorln("Failed to connect to AMQP compatible broker at: " + connectionString)
